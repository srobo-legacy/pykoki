...
class V4LCamera(object):
    ...
    def prepare_buffers(self, count_p):
        ...
        self.buffers = self.libkoki.koki_v4l_prepare_buffers(self.fd, count_p)
        sefl.buffer_count = count_p

    def _free_buffers(self, buffers, count):
        if self.buffers == None:
            return

        self.libkoki.koki_v4l_free_buffers( buffers, self.buffer_count )
        self.buffers = None
        self.buffer_count = 0
...
class PyKoki:
    ...
    def open_camera(self, filename):
        return V4LCamera(filename, self.lib)
